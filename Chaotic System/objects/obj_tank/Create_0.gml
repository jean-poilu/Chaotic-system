...
function coll_line(_c1, _c2) {
	return (collision_line(_c1.x + hspeed, _c1.y + vspeed, _c2.x + hspeed, _c2.y + vspeed, obj_wall, true, false) != noone);
}

function all_lines() {
	if (coll_line(coins[0], coins[1]))
		return true;

	if (coll_line(coins[1], coins[3]))
		return true;

	if (coll_line(coins[3], coins[2]))
		return true;

	if (coll_line(coins[2], coins[0]))
		return true;
